...
class Dataset_Finance_Minute(Dataset):
    def __init__(self, configs, root_path, flag='train', size=None,
                 features='S', data_path='',
                 target='Close', scale=True, timeenc=0, freq='t', percent=100):
    ...
    def __read_data__(self):
        ...
        if self.features == 'M' or self.features == 'MS':
            cols_data = ['Close', 'Volume'] if 'Volume' in df_raw.columns else df_raw.columns[1:]
            df_data = df_raw[cols_data]
        elif self.features == 'S':
            df_data = df_raw[[self.target]]

        if self.scale:
            # Using MinMaxScaler for financial data prices
            train_data = df_data[border1s[0]:border2s[0]]
            self.scaler.fit(train_data.values)
            data = self.scaler.transform(df_data.values)
        else:
