...
class FinancialPatchEmbedding(nn.Module):
    def __init__(self, patch_len, d_model):
        super().__init__()
        self.patch_len = patch_len
        self.d_model = d_model
        # 1D Conv as a low-pass filter over the patch features for high-noise robustness
        self.conv = nn.Conv1d(in_channels=1, out_channels=d_model // 2, kernel_size=3, padding=1)
        self.proj = nn.Linear((d_model // 2) * patch_len, d_model)
        self.act = nn.GELU()
    
    def forward(self, x):
        # x shape: (B, N, P) -> (batch*features), patches, patch_len
        B, N, P = x.shape
        x = x.reshape(B * N, 1, P)
        x = self.act(self.conv(x)) # (B*N, d_model//2, P)
        x = x.reshape(B, N, -1) # (B, N, (d_model//2)*P)
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        # Determine the actual hidden size for the LLM backbone
        if configs.is_llm:
            if "gpt2" in configs.llm:
                self.d_llm = self.gpt.config.n_embd  # 768 for gpt2
            elif "llama" in configs.llm:
                self.d_llm = self.gpt.config.hidden_size
            else:
                self.d_llm = configs.d_model
        else:
            self.d_llm = configs.d_model

        if hasattr(configs, 'data') and configs.data == 'finance':
            self.in_layer = FinancialPatchEmbedding(self.patch_len, self.d_llm)
        else:
            self.in_layer = nn.Linear(self.patch_len, self.d_llm)
        self.out_layer = nn.Linear(self.d_llm, self.patch_len)
